...
- **Text**: `Statischer Text` (static) or `Lauftext` (scrolling), with speed and direction controls. Looping scroll text always repeats with a fixed 2-second pause between loops. When applied across a contiguous multi-large-screen selection, text flows continuously across the combined canvas, treating the real physical gap between screens as blank/phantom pixels so the text doesn't visually jump. The glyph colour (Textfarbe) is chosen from the same shared named preset palette + free colour picker as the Hintergrund tab, and tints the whole text. Text sits on top of the screen's Hintergrund. On large screens, Lauftext's background composites as a static, non-scrolling fill behind the panning glyphs — only the text pans, never the background, which avoids the background visibly sliding or tearing at the loop-pause reset. On small screens this isn't implemented yet (see `docs/adr/0001-phase-lauftext-background-by-hardware-kind.md`): Lauftext there still always renders on black, and the Hintergrund tab shows an inline warning rather than silently doing nothing when a small-screen Lauftext selection has a background chosen. Lauftext is restricted to a 1D-strip selection (single row or single column of large screens) — no defined behavior for scrolling across a 2D block. Hintergrund and Animation/Bild may apply to any contiguous shape (1D or 2D).
- **Animation/Bild**: chosen from a fixed, built-in template library (no user upload in v1) — currently Logo, Smiley, and three genuinely animated templates (a rotating arrow, two differently-timed wobbling raute variants) that carry their own looping motion rather than being a static picture. An "Ohne" option sits alongside the templates and explicitly clears the foreground back to nothing (the screen shows only its Hintergrund) — it's a deselect action, not a template of its own, and is never persisted as a real template id. User can control the template's scale via a percentage control (e.g. 25–400%) — scaling beyond the canvas crops the template rather than being constrained to always fit. Scales/stretches across a contiguous multi-screen selection (rather than tiling/repeating per screen). An animated template's motion is sampled into a frame strip at a fixed 16fps (not a per-edit setting — see `docs/wire-format.md` "frames" and `docs/adr/0002-phase-animated-templates-by-hardware-kind.md`) and today only actually animates on large screens; a small-screen selection shows it as a static frame until ESP32 firmware support lands (`docs/plan-esp32-animation-frames.md`).
- **Hintergrund**: what fills a screen behind everything else — one of a shared named preset palette (brand colours plus Weiß/Schwarz) or a free colour. Changing it recolours _behind_ whatever foreground the screen has, rather than replacing it. There is no separate "off/unlit" state distinct from choosing Schwarz — visually indistinguishable on an LED matrix (both mean no light emitted), so the picker doesn't offer both.
