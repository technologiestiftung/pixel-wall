...
test.describe("homepage", () => {
	test("no selection: should not have any automatically detectable accessibility issues", async ({ page }) => {
		await page.goto("/");
		...
		await page.getByRole("heading", { name: "Pixel Displays Foyer", level: 1 }).waitFor();
	...
	for (const tabName of ["Text", "Animation/Bild", "Farbe"]) {
		test(`${tabName} panel: should not have any automatically detectable accessibility issues`, async ({ page }) => {
			await page.goto("/");
