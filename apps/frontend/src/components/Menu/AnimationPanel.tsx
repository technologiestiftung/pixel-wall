	...
	return (
		<div className="flex w-full flex-col gap-5">
			<div className="flex flex-col gap-2.5">
				<span className="w-full text-[12px] text-[#767671]">Bild oder Animation wählen</span>
					...
					{TEMPLATES.map((template) => {
						...
						return (
							<button
								key={template.id}
								type="button"
								onClick={() => onChange({ ...content, templateId: template.id })}
							...
							>
								...
								>
									<TemplateIcon templateId={template.id} className="h-[22px] w-[22px] text-[#171717]" />
									{selected && <SelectedBadge className="absolute -right-2 -top-2.5 h-4 w-4" />}
								</div>
								<span className={`text-[11px] ${selected ? "text-[#171717]" : "text-[#767671]"}`}>
									{template.label}
								</span>
							</button>
						);
			...
			<div className="flex flex-col gap-2">
				...
				<div className="flex items-center gap-3">
					<input
						...
						onChange={(e) => onChange({ ...content, scalePercent: Number(e.target.value) })}
						className="flex-1"
					/>
