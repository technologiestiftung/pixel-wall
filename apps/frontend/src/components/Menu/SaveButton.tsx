...
export function SaveButton() {
	const { canApply, applyStatus, applyError, handleApply, hasSelection } = useApplyChanges();
		...
		<div className="mt-auto flex flex-col gap-2 pt-4">
			{applyStatus === "error" && (
				<div className="flex items-center justify-between rounded-md bg-red-50 px-3 py-2 text-sm text-red-700">
					...
					<button type="button" onClick={handleApply} className="font-medium underline underline-offset-2">
						Erneut versuchen
					</button>
				</div>
			)}
			<button
				...
				disabled={!canApply}
				onClick={handleApply}
				title={hasSelection ? undefined : "Bildschirme auswählen und Inhalt bearbeiten, um zu speichern"}
